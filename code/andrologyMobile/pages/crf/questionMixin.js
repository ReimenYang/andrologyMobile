  ...
  methods: {
    async questionFormat (question, paper) {
      switch (question.questionType) {
        case '计算':
          ...
          question.mobileCol = [{ key: question.questionId, textContent: question.questionAnswer }]
          break
        case '日期':
          ...
          question.col = {
            ...
            mobileKey: 'datePicker'// 标记移动端保存的时候需要从 mobileCol 获取值更新的字段
          }

          question.mobileCol = [{ key: question.questionId, datePicker: { value: question.questionAnswer } }]
          break
        case '上传图片':
        ...
        case '数字':
          ...
          question.mobileCol = [{ key: question.questionId, number: { value: question.questionAnswer } }]
        ...
        case '单行文本':
          ...
          question.mobileCol = [{ key: question.questionId, input: { value: question.questionAnswer } }]
          break
        case '多行文本':
          ...
          question.mobileCol = [{ key: question.questionId, textarea: { value: question.questionAnswer }, style: { width: '70vw', height: '6em' } }]
          break
        case '单选':
          ...
          question.optionList.forEach(item => { item.label = item.value = item.optionText })
          question.col = {
            label: question.questionTitle,
            prop: 'checked',
            type: question.uiStyle,
          ...
          question.mobileCol = [{ key: question.questionId, radio: { data: question.optionList } }]
        ...
        case '多选':
          ...
          question.mobileCol = [{ key: question.questionId, checkbox: { data: question.optionList } }]
    ...
    async save (patientInfo) {
      ...
      stageList.forEach(stage => {
        ...
        stage.questionnaireList =
          stage.questionnaireList
            ...
            .map(({ questionnaireTypeId, questionnaireTypeName: questionnaireName, paper: groupList }) => {
              ...
              if (isMobile) groupList.map(
                ({ questionList }) => questionList)// 提取问题列表
                .flat()// 变成一维数组
                .filter(({ col }) => col.mobileKey)// 过滤出需要重新赋值 的题目
                .forEach(question => question[question.col.prop] = question.mobileCol[0][question.col.mobileKey].value)// 根据类型字段赋值
              return {
                questionnaireTypeId,
                questionnaireName,
        ...
        console.log(stage.questionnaireList)
      })
