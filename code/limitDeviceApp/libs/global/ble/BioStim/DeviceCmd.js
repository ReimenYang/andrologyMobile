...
//   3-0 发送两条d指令
//   3-1 传入程序参数（m）命令
//   3-2 根据治疗程序的阶段传入（p）命令,ble使用workoutphaselist,优E康使用phaseList
...
/* <PlanNo>，方案序号：1~256
   ...
   * <IsStop> 是否已中止，1是，0否
   * 注：
   * 1. 治疗正常结束时，<Time>应该和<TotalTime>一致，且<IsStop> 为1
   * 2. 治疗过程中，用户手动关闭设备后，再打开设备时，返回的内容：
   * <TotalTime>为设置的时长
   * <Time>为关闭时已治疗的时长
   * <IsStop> 为1
   * 3. 硬件收到新的设置程序指令时，才清空缓存，更新为新一次治疗的数据
   * 4. 如果没有治疗数据：返回ACK:\r\n\0
   * @return
   */
  getRecord () {
